    ...
    <form class="p-4" v-on:submit.prevent="save">
        <div class="modal-card">
            <header class="modal-card-head">
                <h3 class="modal-card-title">Edit Paket</h3>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions('packet', ['update']),
            convert(number) {
                const cost = new Intl.NumberFormat('id-ID').format(number)

                this.packet.cost = cost
            },
            async save() {
                this.loading = true
                this.errors = {...errors}
                ...
                try {
                    const packet = await this.update({
                        id: this.id,
                        data: {
                            ...this.packet,
                            cost: this.rawCost
                    ...
                    this.$emit('saved', 'Paket Diperbarui')
                    this.$emit('close')
                } catch (err) {
